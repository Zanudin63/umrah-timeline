...
const SidebarTab = ({ title, active, color, lightColor, onClick, icon }: SidebarTabProps) => {
  ...
  const preparationsStyle = isPreparations ? "bg-[#112eed] text-white" : "";
    ...
    <div 
      className={cn(
        ...
        active ? (isPreparations ? "bg-[#112eed] text-white" : `${color} text-white`) : 
                 (isPreparations ? "text-white hover:bg-[#112eed]/80 dark:hover:bg-[#112eed]/80" : "hover:bg-muted dark:hover:bg-gray-700")
      )}
      onClick={onClick}
    >
      <div className="mr-3 text-current">{icon}</div>
      <span className="font-medium text-sm uppercase sidebar-title">{title}</span>
...
const JourneySidebar = ({ 
  ...
  return (
    <div className="sticky top-6 h-[calc(100vh-3rem)] flex flex-col pr-2 overflow-y-auto w-[200px] mr-4">
      <div className="mb-6">
        <h3 className="font-bold uppercase mb-2 text-lg px-3 dark:text-white sidebar-title">Journey Phases</h3>
      </div>
